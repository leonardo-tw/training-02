...
function drawResult() {
    ...
    for (let x = 0; x < TILES_X; x++) {
      for (let y = 0; y < TILES_Y; y++) {
        let px = constrain(int(x * tileW + random(-10, 10)), 0, buffer.width - 1);
        let py = constrain(int(y * tileH + random(-10, 10)), 0, buffer.height - 1);
  
        let c = buffer.get(px, py);
  
        // Applica uno spostamento casuale per il glitch
        let glitchX = x + int(random(-2, 2));
        let glitchY = y + int(random(-2, 2));
  
        result.fill(c);
        result.rect(glitchX * tileW, glitchY * tileH, tileW, tileH);
      }
    }
  }
